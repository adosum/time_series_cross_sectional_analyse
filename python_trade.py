...
def validate_config(config):
    ...
    if config['cooldown_after_long_bars'] < 1 or config['cooldown_after_short_bars'] < 1:
...
class SignalTracker:
    ...
    def update(self, candidate_signal):
        ...
        if candidate_signal in ["LONG", "SHORT"]:
            ...
            self.signal_bar_count += 1 # 如果没有信号，或者是过滤/冷静状态，默默增加计数
...
def build_multi_timeframe_features(df_15m):
    ...
    # --- 1. 计算 15m 微观指标 ---
    df['AO_15m'] = AwesomeOscillatorIndicator(window1=5, window2=34, high=df['high'], low=df['low']).awesome_oscillator()
    df['MFI_10'] = MFIIndicator(high=df['high'], low=df['low'], close=df['close'], volume=df['volume'], window=10).money_flow_index()
    df['MFI_14'] = MFIIndicator(high=df['high'], low=df['low'], close=df['close'], volume=df['volume'], window=14).money_flow_index()
    df['MFI_20'] = MFIIndicator(high=df['high'], low=df['low'], close=df['close'], volume=df['volume'], window=20).money_flow_index()
    df['ATR_Fast'] = AverageTrueRange(high=df['high'], low=df['low'], close=df['close'], window=14).average_true_range()
    df['ATR_Slow'] = AverageTrueRange(high=df['high'], low=df['low'], close=df['close'], window=50).average_true_range()
    ...
    df_4h['ADX_4H'] = ADXIndicator(high=df_4h['high'], low=df_4h['low'], close=df_4h['close'], window=14).adx()
    df_4h['AO_4H'] = AwesomeOscillatorIndicator(window1=5, window2=34, high=df_4h['high'], low=df_4h['low']).awesome_oscillator()
    
    # --- 3. 向上合成 1D 数据 ---
    df_1d = df.resample('D').agg(agg_dict).dropna()
    df_1d['ADX_1D'] = ADXIndicator(high=df_1d['high'], low=df_1d['low'], close=df_1d['close'], window=14).adx()
    df_1d['AO_1D'] = AwesomeOscillatorIndicator(window1=5, window2=34, high=df_1d['high'], low=df_1d['low']).awesome_oscillator()
...
# ==========================================
def check_signals(latest_state, config, current_price, signal_tracker, current_positions):
    ...
    critical_indicators = ['ADX_1D', 'ADX_4H', 'AO_1D', 'AO_4H', 'MFI_10', 'MFI_14', 'MFI_20', 'ATR_Fast', 'ATR_Slow']
    for indicator in critical_indicators:
        if pd.isna(latest_state[indicator]):
            return "ERROR", f"[ERROR] 指标 {indicator} 计算异常 (NaN)"
    
    # 获取当前多单和空单的数量
    num_long_pos = len([p for p in current_positions if p.type == mt5.ORDER_TYPE_BUY]) if current_positions else 0
    num_short_pos = len([p for p in current_positions if p.type == mt5.ORDER_TYPE_SELL]) if current_positions else 0
    ...
    mfi_10, mfi_14, mfi_20 = latest_state['MFI_10'], latest_state['MFI_14'], latest_state['MFI_20']

    if vol_ratio > config['volatility_shock_threshold']:
        # 强制覆盖市场状态为 SHOCK
        return "SHOCK", f"⚠️ [CIRCUIT BREAKER] 波动率异常放大 ({vol_ratio:.2f}x)！暂停一切新开仓动作！"

    # ==========================================
    # 仓位风控铁门：震荡期如果有仓位，直接熔断不处理任何新信号！
    # ==========================================
    if market_regime == "RANGE" and total_pos > 0:
        return market_regime, f"[HOLD] 震荡环境已持有 {total_pos} 个仓位，禁止加仓等待获利/止损。"
    ...
    if market_regime == "RANGE":
        ...
        long_votes = sum([mfi_10 < dyn_oversold, mfi_14 < dyn_oversold, mfi_20 < dyn_oversold])
        short_votes = sum([mfi_10 > dyn_overbought, mfi_14 > dyn_overbought, mfi_20 > dyn_overbought])
        ...
        if long_votes > 0 and current_price <= latest_state['BB_Lower'] * 1.001:
            ...
            if expected_rr >= config['min_expected_rr']:
                ...
                signal_msg = f"[FILTERED] Range Long matched, poor RR ({expected_rr:.2f})"
        ...
        # --- 震荡做空逻辑 (突破上轨摸顶) ---
            ...
            if expected_rr >= config['min_expected_rr']:
                ...
                candidate_signal_type = "FILTERED"
                signal_msg = f"[FILTERED] Range Short matched, poor RR ({expected_rr:.2f})"
    ...
    # ==========================================
        ...
        # 【安全检查】验证 Donchian 指标
        donchian_indicators = ['Donchian_Upper_20', 'Donchian_Lower_10', 'Donchian_Lower_20', 'Donchian_Upper_10']
        for ind in donchian_indicators:
            if pd.isna(latest_state[ind]):
                return market_regime, f"[WARNING] Donchian指标 {ind} 暂未准备好"
        
        trend_direction = 1 if (ao_1d > 0 and ao_4h > 0) else (-1 if (ao_1d < 0 and ao_4h < 0) else 0)
        
        # --- 顺大势做多 ---
        if trend_direction == 1:
            # 1. 首次开仓 (底仓)
            if num_long_pos == 0 and num_short_pos == 0:
                if current_price > latest_state['Donchian_Upper_20'] and mfi_14 > config['trend_mfi_confirm']:
                    candidate_signal_type = "LONG"
            ...
            # 2. 盈利加仓 (Pyramiding)
                ...
                highest_entry = max([p.price_open for p in current_positions if p.type == mt5.ORDER_TYPE_BUY])
        ...
        # --- 顺大势做空 (逻辑是对称镜像的) ---
            ...
            if num_short_pos == 0 and num_long_pos == 0:
                if current_price < latest_state['Donchian_Lower_20'] and mfi_14 < short_mfi_confirm:
                    candidate_signal_type = "SHORT"
                ...
                lowest_entry = min([p.price_open for p in current_positions if p.type == mt5.ORDER_TYPE_SELL])
                distance_required = atr_fast * config['trend_add_pos_atr_step']
    ...
    if signal_tracker is not None and candidate_signal_type in ["LONG", "SHORT"]:
        is_cooling, cooldown_remaining = signal_tracker.is_cooldown_active(candidate_signal_type, config)
        
        if is_cooling:
            signal_msg = f"[COOLDOWN] {candidate_signal_type} 信号被冷静期拦截，还需等待 {cooldown_remaining} 根K线"
            candidate_signal_type = "COOLDOWN" # 临时改变状态用于打印，但不影响 tracker 的底层记忆
    ...
    for pos in current_positions:
        ...
        if pos.type == mt5.ORDER_TYPE_BUY:
            ...
            if long_defense_line > current_sl:
                ...
                updates_msg.append(f"多单 #{ticket} SL 上移: {current_sl:.5f} -> {long_defense_line:.5f}")
        ...
        # 处理空单 (SELL)
            ...
            if current_sl == 0.0 or short_defense_line < current_sl:
                # [TODO: 调用 mt5.order_send 发送修改 SL 的指令]
                updates_msg.append(f"空单 #{ticket} SL 下移: {current_sl:.5f} -> {short_defense_line:.5f}")
...
def update_mt5_dashboard(latest_state, market_regime, signal_msg, signal_tracker=None):
    """将状态写入沙盒文件供 MT5 图表显示 (使用纯英文防止乱码)
    
    参数:
        signal_tracker: SignalTracker 对象，用于易表里显示冷静状态
    """
    terminal_info = mt5.terminal_info()
    if terminal_info is None:
        return
        
    file_path = os.path.join(terminal_info.data_path, "MQL5", "Files", "python_dashboard.txt")
    
    text =  "==============================\n"
    text += "   QUANT ENGINE: ACTIVE       \n"
    text += "==============================\n"
    text += f"Time  : {latest_state.name}\n"
...
# ==========================================
def execute_trade(symbol, order_type, lot_size, sl_price, tp_price=0.0, comment="Quant_Engine"):
    """
    ...
    while True:
        try:
            ...
            if last_processed_time != current_bar_time:
                ...
                df_raw = get_mt5_data(symbol, mt5.TIMEFRAME_M15, 1500)
                ...
                if df_raw is not None:
                    ...
                    ts_msg = update_trailing_stops(latest_state, current_positions, market_regime)
                    if ts_msg:
                        print(f"\n[🛡️ 风控系统] {ts_msg}")
                    # 4. 打印到控制台
                    ...
                    update_mt5_dashboard(latest_state, market_regime, signal_msg, signal_tracker)
                
                last_processed_time = current_bar_time

            time.sleep(1) # 1秒休眠防卡死
